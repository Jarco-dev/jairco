...
export class Utilities {
    ...
    public async wordExists(word: string): Promise<boolean> {
        ...
        const checkWord = async (filename: string, text: string) => {
            try {
                const contents = await fsPromises.readFile(filename, 'utf-8')

                return contents.toLowerCase().includes(text)
            } catch (err) {
                this.client.logger.error(
                    `Error while checking word: ${word}`,
                    err
                )
            }
        ...
        let isValid = await checkWord(this.client.config.WORD_LIST_PATH, word.toLowerCase());
